...
void solveLeaf(uint32_t index,
    ...
    float* zy_temp;
    set_const<float>(nstates, 0.0, zy_temp); //checked
               
    if (index ==0) {
        float* Q = &s_Q_R[0]; //CHECK WITH BRIAN!
       ...
       // dont need ti coz we initialized with 0s set_const(nstates*nstates,0, s_F_state); 
        ...
        cholSolve_InPlace<float>(R, s_F_input, false, ninputs, nstates); //ASK XIAN! Fu = R\B
        ...
        glass::scal<float>(nstates,-1.0, s_q_r, cgrps::this_thread_block()); // zx = -zy
        chol_InPlace<float>(nstates, Q); //not sure if that's ok to have it inplace
    ...
    */
    ...
    for(uint32_t level = 0; level < depth; ++level ) {
      ...
      for (uint32_t i = thread_id ; i < num_factors; i+=block_dim) {
        ...
        int index = 0 ; //FIGURE OUT
        bool calc_lambda = shouldCalcLambda(index, k);
        updateShur<float>(s_F_state, s_F_input, s_F_lambda, s_q_r, s_d,  index,  i,  level, upper_level, 
