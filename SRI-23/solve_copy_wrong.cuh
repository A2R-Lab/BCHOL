...

{
  ...
  if (!DEBUG && block_id == BLOCK)
  {
    ...
    float h_A[9] = {
        ...
        15, 55, 225,
        55, 225, 979};
    chol_InPlace<float>(n, h_A, cgrps::this_thread_block());
    block.sync();
  ...
  for (uint32_t level = 0; level < depth; level++)
  {
    ...
    if(DEBUG&&block_id==BLOCK&&thread_id==THREAD){
      printf("Pre-loop check for block %d\n",block_id);
    ...
    for (uint32_t b_ind = block_id; b_ind < numleaves; b_ind += grid_dim)
    {
      ...
      {
        printf("Before calling Fact Inner  %d\n",lin_ind);
        printf("F_state 1: \n");
        printMatrix(s_F_state+index*states_sq,nstates,nstates);
        printf("F_input 1:\n");
        printMatrix(s_F_input+index*inp_states,ninputs,nstates);
        printf("F_state:\n");
        printMatrix(s_F_state+index2*states_sq,nstates,nstates);
        printf("F_lambda\n");
      ...
      block.sync();
        ...
        if (!SAFE_MOOD && block_id == BLOCK && thread_id == THREAD)
      {
        printf("After calling Fact Inner\n");
        printMatrix(s_F_lambda+(states_sq), nstates, nstates);
      }
      block.sync();
      }
    }
