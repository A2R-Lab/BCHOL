...
//Add Dot-Product test and add lower_back sub for matrix and matrix
...
TEST(MatrixFunctionsTest, DiagMatrixSetVTest)
{
    ...
    float *d_v, *d_m;
    gpuErrchk(cudaMalloc((void **)&d_v, N * sizeof(float)));
    gpuErrchk(cudaMalloc((void **)&d_m, N * N * sizeof(float)));
    ...
    gpuErrchk(cudaMemcpy(h_m, d_m, N * N * sizeof(float), cudaMemcpyDeviceToHost));
